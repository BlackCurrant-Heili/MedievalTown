...
{
    // 新增：接收流体速度场纹理
    public RenderTexture velocityFieldTexture { get; set; }
    ...
    [SerializeField] private Camera mainCam;   // 拖主摄像机
    ...
    {
        [Header("草参数")]
        [Range(0.10f, 2f)] public float _HEIGHT;
    ...
    private GrassParams _lastParams; // 缓存用于比较
    public GameObject Collider;
    private bool _isBound = false; // 标记是否已绑定
    ...
    void InitTexture()
    {
        _initGrassTexture = new RenderTexture(Width, Length, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear)
        { enableRandomWrite = true };
        _editGrassTexture = new RenderTexture(Width, Length, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear)
        { enableRandomWrite = true };
    ...
    private void Update()
    {
        ...
        if (velocityFieldTexture == null || sphCellSize <= 0 || sphDimensions <= 0 || sphResolution <= 0)
        {
            // 只在第一帧警告
        ...
        if (!_isBound)
        {
            windCS.SetTexture(_kernelWind, "_VelocityField", velocityFieldTexture);
            windCS.SetFloat("_SphCellSize", sphCellSize);
            windCS.SetInt("_SphDimensions", sphDimensions);
            windCS.SetInt("_SphResolution", sphResolution);
            _isBound = true;
